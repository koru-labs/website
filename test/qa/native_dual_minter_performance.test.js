...
const MINTERS = {
  minter1: {
    ...
    privateKey: '35c285cae6a13a0e13ef7db25776e60b02745922da3b39513b94114c2c5d9add',
  },
  minter2: {
...
async function prepareSplitRequests(round_number, testType = 'transfer') {
  ...
  for (let i = 0; i < round_number; i++) {
    ...
    for (let j = 0; j < 16; j++) {
      if (testType === 'burn') {
        to_accounts.push({ address: MINTERS.minter1.address, amount: 1, comment: 'burn' }, { address: MINTERS.minter1.address, amount: 2, comment: 'burn' });
      } else {
        to_accounts.push(
  ...
  for (let i = 0; i < round_number; i++) {
    ...
    for (let j = 0; j < 64; j++) {
      if (testType === 'burn') {
        to_accounts.push({ address: MINTERS.minter2.address, amount: 1, comment: 'burn' }, { address: MINTERS.minter2.address, amount: 2, comment: 'burn' });
      } else {
        to_accounts.push(
...
async function executeBatchedConcurrentSplits(requests, batchSize = 20) {
  ...
  const minter1TxData = await Promise.all(
    requests.minter1.map(async (requestId, index) => {
      const response = await client.getBatchSplitTokenDetail({ request_id: requestId }, minter1Metadata);
  ...
  const minter2TxData = await Promise.all(
    requests.minter2.map(async (requestId, index) => {
      const response = await client.getBatchSplitTokenDetail({ request_id: requestId }, minter2Metadata);
  ...
  if (pendingTxHashes.length > 0) {
    ...
    for (let i = 0; i < pendingTxHashes.length; i += CONFIRM_BATCH) {
      ...
      await Promise.all(batchHashes.map((hash) => pollReceipt(hash).catch((e) => console.warn(`\nWait failed for ${hash}: ${e.message}`))));
